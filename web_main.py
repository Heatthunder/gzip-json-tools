...
def _infer_json_filename(upload_name: str) -> str:
    ...
    def _to_json_name(stem: str) -> str:
        return stem if stem.lower().endswith(".json") else f"{stem}.json"

    lower_name = upload_name.lower()
    if lower_name.endswith(".gzip"):
        return _to_json_name(upload_name[:-5])
    if lower_name.endswith(".gz"):
        return _to_json_name(upload_name[:-3])
    return "save.json"
